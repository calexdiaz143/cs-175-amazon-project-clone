...
CATEGORIES = [
    ...
    "Musical_Instruments",            # 10,261 # helpful=9745
...
if __name__ == '__main__':
    ...
    memo.SHOW_MESSAGES = SHOW_MESSAGES
    train_X, train_Y, test_X, test_Y, summary_cv, review_cv = memo.get_data(LOAD_SAVED, OVERWRITE_SAVED, CATEGORIES, 0.75, 9500)
    # print(train_X.shape[0] + test_Y.shape[0])
    # clf_svm = memo.get_classifier(LOAD_SAVED, OVERWRITE_SAVED, trainer.svm, train_X, train_Y, 'clf_svm')
    # clf_knn = memo.get_classifier(LOAD_SAVED, OVERWRITE_SAVED, trainer.knn, train_X, train_Y, 'clf_knn')
    # clf_mlp = memo.get_classifier(LOAD_SAVED, OVERWRITE_SAVED, trainer.mlp, train_X, train_Y, 'clf_mlp')
    # clf_ada = memo.get_classifier(LOAD_SAVED, OVERWRITE_SAVED, trainer.adaboost, train_X, train_Y, 'clf_ada')
    ...
    print('{:10.6f}\t{:10.6f}'.format(test_err_nb, train_err_nb))
    print('{:10.6f}\t{:10.6f}'.format(test_err_bnb, train_err_bnb))
    print('{:10.6f}\t{:10.6f}'.format(test_err_lr, train_err_lr))
    print('{:10.6f}\t{:10.6f}'.format(test_err_rf, train_err_rf))
    print('{:10.6f}\t{:10.6f}'.format(test_err_gb, train_err_gb))
    print('{:10.6f}\t{:10.6f}'.format(test_err_ensemble, train_err_ensemble))

    if SHOW_MESSAGES:
        import message
        message.say("The program has finished running.")
